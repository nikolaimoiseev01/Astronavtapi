...
//        ];
    ...
    private function calculateSomeAdditionalDataShares(): void
    {
        ...
        foreach ($someAditionals as $field) {
            foreach ($this->result["some_additional_gates_{$field}"] as $gate => &$group) {
                $total = array_sum(
                    array_map(
                        fn ($item) => count($item['values'] ?? []),
                        $this->result["some_additional_gates_{$field}"]
                    )
                );
                $count = count($group['values'] ?? []);
                $group['share (%)'] = round(($count / $total) * 100, 2);
                $group["total of {$field}"] = $total;
            }
